...
function App() {
  ...
  .map((item) => {
    // Convert timestamp to EST and format date and time
    const timestamp = item.split(" ")[0];
    const estDate = new Date(timestamp).toLocaleString("en-US", options);
    // Return the item with the updated timestamp
    return `${estDate} ${item.slice(20)}`;
  })
  .filter((item) => {
      if (logType === '') {
        return true;
