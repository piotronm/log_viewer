...
const cewVersion = new Set();
const cewVersionRegex = /CEW starting\. CewVersion: (.+)/i;
lines.forEach((line) => {
  const matches = line.match(cewVersionRegex);
  if (matches && matches[1]) {
    cewVersion.add(matches[1]);
  }
});
const cewVersions = Array.from(cewVersion);
setCewVersion(cewVersions)
